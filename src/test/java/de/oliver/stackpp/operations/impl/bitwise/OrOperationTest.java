...
class OrOperationTest {
    ...
    @Test
    void execute() {
        Operation operation = new OrOperation(program, 1, p -> p.getMachine().getIntegerRegister("a"),  p -> p.getMachine().getIntegerRegister("b").getValue());
        operation.execute();

        assert program.getMachine().getIntegerRegister("a").getValue() == 0b01010101;
    }
}